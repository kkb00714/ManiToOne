class ManitoLetterModal extends BaseModal {
  ...
  initializeSendConfirmation() {
    ...
    if (confirmationPopup && successPopup && confirmBtn && cancelBtn && successConfirmBtn) {
...
const ManitoPage = {
  ...
  letterBox: {
    ...
    init() {
      ...
      this.scrollContainer = document.querySelector('.manito-letter-box-container');
    ...
    setupEventListeners() {
      this.receivedTab.addEventListener('click', () => {
        ...
        document.querySelector('.manito-letter-box-switch').classList.remove('sent-active');
        document.querySelector('.manito-letter-box-switch').classList.add('received-active');
      });
      ...
      this.sentTab.addEventListener('click', () => {
        ...
        document.querySelector('.manito-letter-box-switch').classList.remove('received-active');
        document.querySelector('.manito-letter-box-switch').classList.add('sent-active');
      });
    ...
    async switchTab(tabName) {
      if (this.activeTab === tabName || this.isLoading) return;
    ...
    async loadMoreLetters() {
      ...
      try {
        const userNickname = document.querySelector('meta[name="user-nickname"]')?.content;
        if (!userNickname) throw new Error('User nickname not found');

        const endpoint = this.activeTab === 'received'
            ? `/api/receivemanito/${userNickname}`
            : `/api/sendmanito/${userNickname}`;

        const response = await fetch(`${endpoint}?page=${this.currentPage}&size=10`);
    ...
    async createLetterHTML(letter) {
      try {
        const response = await fetch(`/view/fragments/manito-letter?letterId=${letter.manitoLetterId}`);
        if (!response.ok) throw new Error('Network response was not ok');
        return await response.text();
      } catch (error) {
        console.error('Error fetching letter fragment:', error);
        return '';
      }
    },
